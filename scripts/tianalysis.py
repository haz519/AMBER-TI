...
class AlchemicalTransform(object):
    ...
    def get_beta(self,temperature=298.):
    ...
    def _get_mbar_arrays(self,time0,time1,temperature=298.,dvdl_autocor=True,uncorrelate=False):
        #print (time0,time1)
        if uncorrelate:
            states = []
    ...
    def get_energies(self,tequil=0,tmax=1.e+10,temperature=298.,dvdl_autocor=True,overlap=""):
        return self._get_energies_in_range(tequil,tmax,temperature,dvdl_autocor,overlap=overlap)
        
    
    def get_forward_timeseries(self,tequil=0,dtime=10000,temperature=298.,dvdl_autocor=True,verbose=False):
        tseries = ddict( float )
    ...
    def get_reverse_timeseries(self,tequil=0,dtime=10000,temperature=298.,dvdl_autocor=True,verbose=False):
        tseries = ddict( float )
        maxtime = self.get_maxtime()
        if tequil >= maxtime:
            return tseries
    ...
    def get_segmented_timeseries(self,tequil=0,dtime=10000,temperature=298.,dvdl_autocor=True,verbose=False):
        tseries = ddict( float )
        maxtime = self.get_maxtime()
        if tequil >= maxtime:
            return tseries
